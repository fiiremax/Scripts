...
function OrionLib:SetOTheme()
	...
	for typeName, objects in next, self.ThemeObjects do
		local color = thm[typeName] or (typeName == "Accent2" and thm.Accent)
		if color then
...
function UnlockMouse(Value) --! AddSectionnnn 
		...
		local function Getelmnts(ItemParent, _tabName)
			...
			function ElementFunction:ColorLabel(Text, ToChangeColor, Position)
				...
				if not ToChangeColor then
					AddThemeObject(ContentLabel, "Text")
				end
				...
				function labelfunc:Set(ToChange, ToChangeColor, Position)
					...
					if ToChangeColor then
						...
						AddThemeObject(LabelFrame.Content, "Text")
					end
			...
			function ElementFunction:AddToggle(ToggleConfig)
				...
				if hasBind then --biiiiiiiiiiiind
